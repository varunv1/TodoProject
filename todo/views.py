    ...
    if request.method == 'GET':
        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                return render(request, 'todo/signupuser.html', {'form': UserCreationForm(), 'error':'Username has already been taken, please choose a new one.'})
        else:
...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user == None:
            return render(request, 'todo/loginuser.html', {'form': AuthenticationForm(), 'error': 'Username and password do not match. Please enter correct credentials.'})
        else:
            login(request, user)
            return redirect('currenttodos')
@login_required
